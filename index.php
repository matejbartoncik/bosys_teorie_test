...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'submit') {
    $selectedIds = json_decode($_POST['selected_topic_ids'] ?? '[]', true);
    ...
    $topicsById = array_column($topics, null, 'id');
    ...
    foreach ($selectedIds as $topicId) {
        ...
        foreach ($topic['questions'] as $qi => $q) {
            $fieldName = "answer_{$topicId}_{$qi}";
            $answer = trim($_POST[$fieldName] ?? '');
            $topicEntry['sub_questions'][] = [
                'question' => $q['text'],
                'points_max' => $q['points'],
                'answer' => $answer,
            ];
        }
        $result['questions'][] = $topicEntry;
    }

    $answersDir = __DIR__ . '/answers';
    if (!is_dir($answersDir)) {
        mkdir($answersDir, 0755, true);
    }
    $filename = 'pokus_' . date('Ymd_His') . '.json';
...
// --- Select 4 random topics for the form ---
if (!isset($savedFile)) {
    $allIds = array_keys($topics);
    shuffle($allIds);
    $selectedIndices = array_slice($allIds, 0, 4);
    $selectedTopics = array_map(fn($i) => $topics[$i], $selectedIndices);
}
?>
<!DOCTYPE html>
<html lang="cs">
...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .btn-primary {
    ...
    color: #fff;
    box-shadow: 0 3px 10px rgba(15,52,96,.25);
  }
  ...
  .review-body { padding: 1rem 1.5rem; }

  .review-q {
    margin-bottom: 1.25rem;
  }

  .review-q:last-child { margin-bottom: 0; }
...
<?php if (isset($savedFile)): ?>
  ...
  <div class="success-box">
    ...
    <h2>Test odevzdán!</h2>
    <p>Soubor s odpověďmi byl uložen do:</p>
    <span class="filename">answers/<?= htmlspecialchars($savedFile) ?></span>
  </div>

  <div class="ai-hint">
    <strong>Jak nechat test obodovat AI:</strong><br>
    Obsah souboru <code>answers/<?= htmlspecialchars($savedFile) ?></code> zkopíruj do chatbotu a přidej pokyn:<br><br>
  ...
  <?php foreach ($savedData['questions'] as $qi => $topicEntry): ?>
  <div class="review-card">
    <div class="review-topic-header">
      Otázka <?= $qi + 1 ?>: <?= htmlspecialchars($topicEntry['topic_title']) ?>
    </div>
    <div class="review-body">
      <?php foreach ($topicEntry['sub_questions'] as $sq): ?>
      <div class="review-q">
        <div class="q-text">
          <?= htmlspecialchars($sq['question']) ?>
          <span class="badge"><?= $sq['points_max'] ?> <?= $sq['points_max'] === 1 ? 'bod' : ($sq['points_max'] < 5 ? 'body' : 'bodů') ?></span>
        </div>
        <?php if ($sq['answer'] !== ''): ?>
          <div class="a-text"><?= htmlspecialchars($sq['answer']) ?></div>
        <?php else: ?>
  ...
  <div class="actions">
    <a href="index.php" class="btn btn-primary">&#8635; Nový test</a>
  </div>
...
<?php else: ?>
  ...
  <form method="POST" action="index.php">
    ...
    <input type="hidden" name="selected_topic_ids" value="<?= htmlspecialchars(json_encode(array_column($selectedTopics, 'id'))) ?>">

    <?php foreach ($selectedTopics as $ti => $topic): ?>
    <div class="topic-card">
      ...
      <div class="topic-body">
        <?php foreach ($topic['questions'] as $qi => $q): ?>
          <?php if ($qi > 0): ?><hr class="divider"><?php endif; ?>
          <div class="sub-question">
            <label for="answer_<?= $topic['id'] ?>_<?= $qi ?>">
              <span><?= htmlspecialchars($q['text']) ?></span>
              <span class="badge"><?= $q['points'] ?> <?= $q['points'] === 1 ? 'bod' : ($q['points'] < 5 ? 'body' : 'bodů') ?></span>
            </label>
            <textarea
              id="answer_<?= $topic['id'] ?>_<?= $qi ?>"
              name="answer_<?= $topic['id'] ?>_<?= $qi ?>"
    ...
    <div class="actions">
      <a href="index.php" class="btn btn-secondary">&#8635; Jiné otázky</a>
